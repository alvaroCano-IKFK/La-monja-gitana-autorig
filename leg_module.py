...
class LegModule(object):
    ...
    def build(self):
        ...
        # El lado R necesita orientación inversa
        sao = "xup" if self.side == "R" else "xdown"
        ...
        # ---- 4. IK SETUP ----
        # Preferred angle para que el solver sepa hacia dónde doblar
        pref_rot = -0.1 if self.side == "L" else 0.1
        cmds.setAttr(f"{self.ik_chain[1]}.rotateZ", pref_rot)
        cmds.joint(self.ik_chain[0], edit=True, ch=True, spa=True)
        cmds.setAttr(f"{self.ik_chain[1]}.rotateZ", 0)
        ...
        ik_footBall, _ = cmds.ikHandle(
            sj=self.ik_chain[2], ee=self.ik_chain[3],
            sol="ikSCsolver", n=f"{self.prefix}_footBall_HDL"
        )
        ik_footTip, _ = cmds.ikHandle(
            sj=self.ik_chain[3], ee=self.ik_chain[4],
        ...
        fk_ctrls = []
        fk_gens  = []
        for i in range(4):   # thigh, knee, ankle, ball
            jnt       = self.fk_chain[i]
            ctrl_name = f"{self.prefix}_{self.names[i]}_fk_CTRL"
        ...
        # ---- JERARQUÍA DE CONTROLES DEL PIE ----
        # heel → bankIn → bankOut → tip → ball  (envuelven los IK handles)
        ...
        # Switch offset lateral
        offset_t = (10 if self.side == "L" else -10)
        cmds.xform(switch_gen, r=True, os=True, t=(offset_t, 0, 0))
        ...
        # ---- 7. PAIR BLENDS (thigh, knee, ankle) ----
        for i in range(3):
