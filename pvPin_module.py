...
class Pv_pin(object):
    ...
    def setup_pole_vector_pin(self, ik_control, root_control, pole_vector_control,
        ...
        """
        Crea el sistema de PV Pin seguint les convencions de nomenclatura del projecte.
        """
        print(f"[PV Pin] Iniciant setup per: {self.side}_{self.name}")  # <- debug temporal
        ...
        cmds.connectAttr(f"{soft_condition_node}.outColorG", f"{blend_upper}.input[0]", force=True)
        cmds.connectAttr(f"{soft_condition_node}.outColorB", f"{blend_lower}.input[0]", force=True)
