...
class Pv_pin(object):
    def __init__(self, side="L", name="leg"):
        # DEFINIM LES VARIABLES DE CLASSE REQUERIDES
        self.side = side
        self.name = name
        self.pin_nodes = []
   
    def setup_pole_vector_pin(self, ik_control, root_control, pole_vector_control, soft_trn, soft_condition_node, upper_ik_joint, lower_ik_joint):
        """
        Crea el sistema de PV Pin seguint les conventions de nomenclatura del projecte.
        """
        ...
        cmds.connectAttr(f"{root_control}.worldPosition[0]", f"{dist_root_to_pv}.point1")
        cmds.connectAttr(f"{pole_vector_control}.worldPosition[0]", f"{dist_root_to_pv}.point2")
        ...
        cmds.connectAttr(f"{soft_trn}.worldPosition[0]", f"{dist_soft_to_pv}.point1")
        cmds.connectAttr(f"{pole_vector_control}.worldPosition[0]", f"{dist_soft_to_pv}.point2")
        ...
        cmds.connectAttr(f"{ik_control}.pin", f"{blend_upper}.attributesBlender")
        cmds.connectAttr(f"{ik_control}.pin", f"{blend_lower}.attributesBlender")

        cmds.connectAttr(f"{soft_condition_node}.outColorG", f"{blend_upper}.input[0]")
        cmds.connectAttr(f"{soft_condition_node}.outColorB", f"{blend_lower}.input[0]")

        cmds.connectAttr(f"{dist_root_to_pv}.distance", f"{blend_upper}.input[1]")
        cmds.connectAttr(f"{dist_soft_to_pv}.distance", f"{blend_lower}.input[1]")
